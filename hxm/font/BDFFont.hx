...
class BDFFont extends Font{
        ...
        while (lines.length>0){
            ...
            switch (prop){
                case 'FAMILY_NAME':{
                    ...
                    trace('Font name: '+this.name);
                }
                case 'SIZE':{
                    this.size = this.initSize = extractInt(args[0]);
                    trace('Font size: '+this.size);
                }
                case 'BITS_PER_PIXEL':{
                ...
                case 'FONTBOUNDINGBOX':{
                    ...
                    trace('Font bounding box height: '+fbbHeight);
                }
                case 'FONT_ASCENT':{
                    this.ascent = extractInt(args[0]);
                    trace('Font ascent: '+ascent);
                }
                case 'FONT_DESCENT':{
                    this.descent = extractInt(args[0]);
                    trace('Font descent: '+descent);
                }
            ...
            if (processingGlyphHeader){
                switch (prop){
                    case 'ENCODING':{
                        ...
                        trace('  Character: '+encoding+' '+String.fromCharCode(encoding));
                    }
                    case 'DWIDTH':{ // Device width
						...
						trace('  Device width (stride): '+stride);
                    }
                    case 'BBX':{
                        bbxFound = true;
                        ...
                        trace('  Bounding box: '+bbxWidth+'x'+bbxHeight+' '+bbxXOffset+','+bbxYOffset); 
                    }
                    case 'BITMAP':{
    ...
    private function generateGlyphs(){
        ...
        var bitmapWidth : Int32 = Math.ceil(Math.sqrt(volume*(1+BDFFont.BitmapPad)));
        trace('Bitmap volume: '+volume);
        trace('Bitmap size: '+bitmapWidth+'x'+bitmapWidth);
        if (bitmapWidth>1024) throw('The font bitmap is too big: '+bitmapWidth+'x'+bitmapWidth);
        
        // Create the bitmap
        var bitmapData : BitmapData = new BitmapData( bitmapWidth, bitmapWidth );
        bitmapData.lock();
        bitmapData.clear(BDFFont.ClearColor); // Blue, but transparent
        ...
        for (d in glyphData){
            ...
            while (x<=(bitmapWidth-d.width)){
                ...
                while (y<=(bitmapWidth-d.height)){
                    ...
                    if (bitmapData.getPixel(x,y)==BDFFont.ClearColor){
                        found = true;
                        // Check first row and first column are clear to ensure space is clear
                        for (xx in x...(x+d.width)){
                            if (bitmapData.getPixel(xx,y)!=BDFFont.ClearColor){
                                found = false;
                                break;
                            }
                        }
                        if (found){
                            for (yy in y...(y+d.height)){
                                if (bitmapData.getPixel(x,yy)!=BDFFont.ClearColor){
                                    found = false;
                                    break;
                                }
                            }
            ...
            for (yy in y...(y+d.height)){
                ...
                for (xx in x...(x+d.width)){
                    ...
                    bitmapData.setPixel( xx, yy, pixAlpha | BDFFont.PixelColor);
                    // Advance
                    pixLeftInByte--;
                }
            }
