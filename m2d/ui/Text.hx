...
class Text extends Canvas{
	...
	*/
	...
	function get_heightInLines() : Int{
		if (needResync) sync_post( null );
		var ls : Float = font.lineHeight*(lineSpacing-1);	// Line spacing
		var ch : Float = elementRect.height + ls;		
	...
	function set_heightInLines( v : Int ) : Int{
		var ls : Float = font.lineHeight*(lineSpacing-1);	// Line spacing
		elementRect.height = v * (font.lineHeight+ls) - ls;
		autoHeight = false;
		invalidate();
		return v;
	}
	...
	override function sync_post( ctx : RenderContext ){
		...
		if (lines==null){
			...
			if (autoWidth) mw = Math.floor(contentRect.width);
			else mw = Math.floor(contentRect.width);
...
//trace('    Remaining line from $si to $i');				
			...
			contentRect.height = linesHeight;
		}
		needRedraw = true;
	}
	...
	override function draw_post( ctx : RenderContext ){
		...
		glyphs.x = contentRect.x;
		glyphs.y = contentRect.y;
		glyphs.draw( ctx );
	}
